...
import { isGlassBreakDetected, isTampered, signalLevelToPercent } from '../../lib/util';

module.exports = class GlassBreakDetectorDevice extends AjaxBaseDevice {

  private deviceListenerBound: ((data: any) => void) | null = null;
  private dataUpdatedBound: (() => void) | null = null;

  async onInit(): Promise<void> {
    this.log('Glass break detector init:', this.getName());
  ...
  private async updateCapabilities(device: AjaxDevice): Promise<void> {
    await this.safeSetCapability('ajax_glass_break', isGlassBreakDetected(device));
