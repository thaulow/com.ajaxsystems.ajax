...
import { isSmokeDetected, isHeatDetected, isCODetected, isTampered, signalLevelToPercent } from '../../lib/util';

module.exports = class SmokeDetectorDevice extends AjaxBaseDevice {

  private deviceListenerBound: ((data: any) => void) | null = null;
  private dataUpdatedBound: (() => void) | null = null;

  async onInit(): Promise<void> {
    this.log('Smoke detector init:', this.getName());
  ...
  private async updateCapabilities(device: AjaxDevice): Promise<void> {
    await this.safeSetCapability('alarm_smoke', isSmokeDetected(device));
    await this.safeSetCapability('alarm_heat', isHeatDetected(device));
    await this.safeSetCapability('alarm_co', isCODetected(device));
